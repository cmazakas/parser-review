...
template <typename... T> using void_t = typename void_<T...>::type;
...
template <typename Default, template <typename...> class Template,
          typename... Args>
...
template <typename T> using has_end = decltype(*std::end(std::declval<T>()));
...
using is_range_of_char = std::integral_constant<
    bool, (std::is_convertible<BeginValueType, const char>::value &&
...
inline std::size_t hash_combine_(std::size_t seed, std::size_t value) noexcept {
...
template <typename T> struct uniq_ptr {
  using pointer = T *;

  uniq_ptr() = default;
  explicit uniq_ptr(T *ptr) : ptr_(ptr) {}

  uniq_ptr(uniq_ptr &&other) : ptr_(other.ptr_) { other.ptr_ = nullptr; }
  template <typename U,
            typename Enable = std::enable_if_t<
                std::is_convertible_v<typename uniq_ptr<U>::pointer, pointer>>>
...
private:
  ...
  template <typename U> friend struct uniq_ptr;
};

template <typename T, typename... Args> auto make_uniq(Args &&...args) {
...
template <typename T> struct value_impl;

template <typename T>
struct is_object : is_range_of<T, std::pair<std::string, value>,
                               std::pair<std::string const, value>> {};

template <typename T> struct is_array : is_range_of<T, value> {};
...
struct value {
  ...
  bool operator==(value const &rhs) const noexcept {
    ...
    case value::boolean_k:
      return storage_.local_.bytes_[3] == rhs.storage_.local_.bytes_[3];
    case value::number_k:
      return *double_ptr() == *rhs.double_ptr();
    case value::local_string_k:
      return strcmp(storage_.local_.bytes_.data(),
                    rhs.storage_.local_.bytes_.data()) == 0;
    default:
      return storage_.remote_.ptr_->equal_impl(rhs);
    }
    return false;
  }

  bool operator!=(value const &rhs) const noexcept { return !(*this == rhs); }
...
private:
  void copy_impl(value const &other) {
    if (other.is_local()) {
      ...
      storage_.remote_.ptr_ = other.storage_.remote_.ptr_->copy_impl();
    }
  }
  ...
  bool is_local() const noexcept { return storage_.local_.kind_ < object_k; }

  double const *double_ptr() const noexcept {
    double const *retval =
        reinterpret_cast<double const *>(storage_.local_.bytes_.data() + 7);
...
namespace detail {
template <> struct value_impl<object> : value_impl_base {
  ...
  virtual detail::uniq_ptr<value_impl_base> copy_impl() const override {
    return detail::uniq_ptr<value_impl_base>(new value_impl<object>(*this));
  }
  ...
  virtual std::ostream &to_json(std::ostream &os) const noexcept override;

  object value_;
};

template <> struct value_impl<array> : value_impl_base {
  template <typename Iter>
  value_impl(Iter first, Iter last) : value_(first, last) {}
  value_impl(array &&a) : value_(std::move(a)) {}

  virtual detail::uniq_ptr<value_impl_base> copy_impl() const override {
    return detail::uniq_ptr<value_impl_base>(new value_impl<array>(*this));
  }
...
std::ostream &write_escaped_cps(std::ostream &os, CPSubRange subr) noexcept {
  ...
  for (auto cp : subr) {
    ...
    default:
      if (cp < 0x20) {
        ...
        auto const hi = static_cast<uint16_t>(cp >> 10) + high_surrogate_base;
        auto const lo = static_cast<uint16_t>(cp & 0x3ff) + low_surrogate_base;
        os << "\\u" << std::setw(4) << hi;
        os << "\\u" << std::setw(4) << lo;
      }
      break;
...
inline std::ostream &to_json(std::ostream &os, std::string_view s) noexcept {
  os << '"';
  ...
  boost::parser::detail::text::foreach_subrange_if(
      ...
      [&](auto subr) {
        os.write(last_written_it, &*subr.begin().base() - last_written_it);
...
template <> struct value_impl<std::string> : value_impl_base {
  template <typename Iter> value_impl(Iter f, Iter l) : value_(f, l) {}
...
std::ptrdiff_t fast_distance_or_1000(Iter f, Sentinel l, CategoryTag tag) {
  return 1000;
}

template <typename Iter>
std::ptrdiff_t fast_distance_or_1000(Iter f, Iter l,
...
template <> struct get_impl<object> {
  static object const &call(value const &v) noexcept {
    assert(v.is_object());
    return static_cast<value_impl<object> *>(v.storage_.remote_.ptr_.get())
        ->value_;
  }
...
template <> struct get_impl<array> {
  static array const &call(value const &v) noexcept {
    assert(v.is_array());
    return static_cast<value_impl<array> *>(v.storage_.remote_.ptr_.get())
        ->value_;
  }
...
template <> struct get_impl<double> {
...
template <> struct get_impl<std::string_view> {
...
template <> struct get_impl<null_t> {
...
template <typename T> detail::get_result_t<T, false> get(value &v) noexcept {
  return detail::get_impl<T>::call(v);
}

template <typename JSONObject>
value::value(JSONObject const &o,
             typename std::enable_if<detail::is_object<JSONObject>::value>::type
                 **enable) {
  storage_.remote_ =
      remote{object_k,
...
inline value::value(object &&o) {
  storage_.remote_ = remote{
      object_k, detail::make_uniq<detail::value_impl<object>>(std::move(o))};
}

template <typename JSONArray>
value::value(JSONArray const &a,
             typename std::enable_if<detail::is_array<JSONArray>::value>::type *
                 *enable) {
  storage_.remote_ =
      remote{array_k,
...
value::value(
    String const &str,
    typename std::enable_if<detail::is_string<String>::value>::type **enable) {
...
inline value &value::operator=(std::string &&s) {
  if (is_string()) {
    if (is_local() && s.size() <= 14u) {
      *std::copy(s.begin(), s.end(), storage_.local_.bytes_.data()) = 0;
    } else {
...
inline std::ostream &operator<<(std::ostream &os, value const &value) {
  ...
  case value::number_k:
    return os << *value.double_ptr();
  case value::local_string_k:
    return detail::to_json(os, value.storage_.local_.bytes_.data());
  default:
    return value.storage_.remote_.ptr_->to_json(os);
  }
  return os;
}
...
inline std::size_t hash_append(std::size_t seed, value const &v) {
  ...
  case value_kind::object:
    return hash_append(kind_hash, get<object>(v));
  case value_kind::array:
    return hash_append(kind_hash, get<array>(v));
  case value_kind::number:
    return hash_combine_(kind_hash, std::hash<double>{}(get<double>(v)));
  case value_kind::string:
    return hash_combine_(
        kind_hash, std::hash<std::string_view>{}(get<std::string_view>(v)));
...
inline std::size_t hash_append(std::size_t seed, object const &o) {
  ...
  for (auto const &pair : o) {
    retval = hash_combine_(retval, std::hash<std::string>{}(pair.first));
...
inline std::size_t hash_append(std::size_t seed, array const &a) {
  std::size_t retval = std::hash<size_t>{}(std::size_t(value_kind::array));
...
size_t hash<json::object>::operator()(argument_type const &o) const noexcept {
