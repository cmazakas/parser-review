...
namespace detail {
template <typename T, bool Const> struct get_result {
  using type = T;
};
template <> struct get_result<array, true> {
  using type = array const &;
};
template <> struct get_result<array, false> {
  using type = array &;
};
template <> struct get_result<object, true> {
  using type = object const &;
};
template <> struct get_result<object, false> {
  using type = object &;
};
